...
export function registerJobCommand(program) {
    ...
    job
        ...
        const rows = jobs.map((j) => {
            ...
            return {
                ...
                runnerAlive: j.pid !== undefined && (j.status === 'running' || j.status === 'pending')
                    ? alive
                    : undefined,
            };
        });
    ...
    job
        ...
        try {
            const json = job.optsWithGlobals().json;
            const outcome = await checkJob({ jobId, maxOutputLength: options.maxOutput ? Number(options.maxOutput) : undefined }, deps);
            if (outcome.ok && !json) {
                const d = outcome.data;
                console.error(`${d.jobId}: ${d.status} (elapsed ${Math.round(Number(d.elapsedMs) / 1000)}s)`);
                const text = d.partialOutput ?? d.result?.stdout ?? '';
                if (text)
    ...
    job
        ...
        try {
            ...
            process.exitCode = report(outcome, json, (d) => console.log(`${d.jobId}: ${d.message}`));
        }
        finally {
            cleanupCli(deps);
        }
