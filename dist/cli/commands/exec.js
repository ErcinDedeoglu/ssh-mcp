...
export function registerExecCommand(program) {
    const cmd = program
        ...
        .argument('<command...>', 'command to execute (quote as one string for shell operators, like ssh)')
    ...
    cmd.action(async (serverId, commandParts, options) => {
        const command = commandParts.join(' ');
        const json = cmd.optsWithGlobals().json;
        const timeout = options.timeout !== undefined ? Number(options.timeout) : undefined;
        const stallTimeout = options.stallTimeout !== undefined ? Number(options.stallTimeout) : undefined;
        if (options.bg) {
