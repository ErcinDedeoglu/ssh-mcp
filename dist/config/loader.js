...
function checkPermissions(filePath) {
    ...
    if (hasGroupOrOthersAccess) {
        throw new Error(`Insecure file permissions on ${filePath}. ` +
...
function loadSingleConfig(configPath, options) {
    if (!fs.existsSync(configPath)) {
        if (options.createTemplate) {
            createTemplateConfig(configPath);
            throw new Error(`Config file not found. Created template at ${configPath} - edit it with your servers and restart.`);
        }
...
export function getServerConfigPath(serverId) {
