...
export async function checkJob(input, deps) {
    try {
        ...
        if (job.result) {
            const { text: stdout, truncated } = truncateOutput(job.result.stdout, effectiveMaxOutputLength);
        ...
        if (job.output) {
            const { text: partialOutput, truncated } = truncateOutput(job.output, effectiveMaxOutputLength);
...
function resolveJob(jobId, deps) {
...
function cleanStreamedOutput(raw) {
