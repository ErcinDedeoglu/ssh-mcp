...
export function registerExecuteTool(server, config, pool, forwardRegistry, shellRegistry) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    server.tool('execute', 'Execute a shell command on a connected SSH server. State (cwd, env vars) persists across calls. ' +
        ...
        stdin: z
            .string()
            .optional()
            .describe('Content to write to the command stdin. Use for commands that read from stdin ' +
        ...
        stallTimeout: z
            ...
            .nullable()
            .optional()
            .describe('Stall timeout in seconds - max time without output before failing. ' +
        ...
        agentForward: z
            .boolean()
            .optional()
            .describe('Enable SSH agent forwarding for this shell session. ' +
        ...
        const outcome = await executeCommand(input, partialDeps({ config, pool, forwardRegistry, shellRegistry }));
