...
export async function runJob(
  ...
  const onSignal = () => {
    ...
    if (shell?.hasRunningCommand) shell.cancelCurrentCommand();
    store.save({
      ...meta,
      status: 'cancelled',
  ...
  try {
    ...
    const { shell } = await getOrCreateShell(
      ...
      deps.shellRegistry,
      { shellType: connection.serverConfig.shell, serverConfig: connection.serverConfig },
    );

    const result = await shell.execute(command, {
      timeoutMs: resolveTimeoutMs(options.timeout, connection.serverConfig, deps.config),
      stallTimeoutMs:
        options.stallTimeout === undefined
          ? undefined
