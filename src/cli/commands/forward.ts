...
export function registerForwardCommands(program: Command): void {
  const forward = program
    .command('forward')
    .description(
      'Forward a local port to a remote host:port through the SSH server (runs in foreground)',
    )
  ...
  forward.action(async (serverId: string, remoteHost: string, remotePort: number, options) => {
  ...
  const rforward = program
    ...
    .option('--via <jumpServerId>', 'connect through a jump host first');
  rforward.action(async (serverId: string, localHost: string, localPort: number, options) => {
  ...
  forwards.action(() => {
    ...
    const json = forwards.optsWithGlobals().json as boolean;
    if (json) {
  ...
  program
    ...
    .action((localPort: number, options) => {
      signalOwner(
        new ForwardStore()
          .list()
          .filter(
  ...
  program
    ...
    .action((serverId: string, remotePort: number, options) => {
      signalOwner(
        new ForwardStore()
          .list()
          .filter(
...
interface ForegroundForwardSpec {
  kind: 'local' | 'remote';
  serverId: string;
  via?: string;
  action: (
    deps: ReturnType<typeof buildCliDeps>,
  ) => Promise<import('../../actions/types.js').ActionOutcome>;
}

async function runForegroundForward(cmd: Command, spec: ForegroundForwardSpec): Promise<void> {
  ...
  if (registered) store.removeByPid(process.pid);
  cleanupCli(deps);
}

function signalOwner(entries: import('../forward-store.js').ForwardEntry[]): void {
  if (entries.length === 0) {
